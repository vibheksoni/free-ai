...
const VERTEX = `
  ...
  void main() {
    ...
    vColor = mix(vec3(0.18), vec3(0.93, 0.36, 0.13), smoothstep(38.0, 10.0, depth) * (0.2 + uScroll * 0.4));
  }
`;
...
export default function CanvasScroller() {
  ...
  onMount(() => {
    ...
    const gridMat = new THREE.PointsMaterial({
      color: 0x2a2a2a, size: 0.06, transparent: true, opacity: 0.3,
    ...
    const accMat = new THREE.PointsMaterial({
      color: 0xee5d20, size: 0.12, transparent: true, opacity: 0.5,
      blending: THREE.AdditiveBlending, depthWrite: false,
    });
    ...
    onCleanup(() => {
      ...
      geometry.dispose(); gridGeo.dispose(); accGeo.dispose();
      material.dispose(); gridMat.dispose(); accMat.dispose();
      renderer.dispose();
      if (container) container.innerHTML = "";
    });
  });

  return <div ref={container} style={{ position: "fixed", inset: 0, pointerEvents: "none", zIndex: 0, opacity: 0.55 }} />;
}
